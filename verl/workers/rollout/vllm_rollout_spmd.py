...
from ...utils.tag_logit_processor import create_tag_logit_processor
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # Store tokenizer for tag restriction
    ...
    def _apply_tag_restriction_post_process(
        ...
        Note: This method corrects invalid tokens after generation, which is less efficient
        than preventing them during generation. For proper implementation, vLLM's logits_processor
        interface should be used.
        """
        if self.tag_logit_processor is None:
            return completions
        ...
        for completion in completions:
            ...
            for output in completion.outputs:
                # Get token_ids - vLLM stores them as a list
                token_ids = list(output.token_ids) if hasattr(output, 'token_ids') else []
                
                # Check if <TAG> appears and correct the next token if needed
                corrected = False
                ...
                if corrected:
                    ...
                    try:
                        # vLLM's CompletionOutput may allow direct modification
                        if hasattr(output, 'token_ids'):
                            if isinstance(output.token_ids, list):
                                output.token_ids = token_ids
                            else:
                                # If immutable, try to set via attribute
                                object.__setattr__(output, 'token_ids', token_ids)
                    except Exception:
                        # If modification fails, we'll proceed with original
                        # The restriction enforcement may be incomplete
                        pass
                
                corrected_outputs.append(output)
            
            # Update completion with corrected outputs
            try:
                if hasattr(completion, 'outputs'):
                    completion.outputs = corrected_outputs
            except Exception:
                pass
            
            corrected_completions.append(completion)
