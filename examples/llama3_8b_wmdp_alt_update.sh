...
DEBUG_DATA=/data/wenjie_jacky_mo/EasyR1/data/tag_no_tag_split/lineage
NORMAL_DATA=/data/wenjie_jacky_mo/EasyR1/data/tag_no_tag_split/no_tag

python3 -m verl.trainer.main \
    config=examples/config.yaml \
    data.alt_update=true \
    data.debug_train_files=${DEBUG_DATA}/train.json \
    data.normal_train_files=${NORMAL_DATA}/train.json \
    data.val_files=${NORMAL_DATA}/val.json \
    data.prompt_key=prompt \
    data.answer_key=answer \
    data.format_prompt=null \
    worker.actor.model.model_path=${MODEL_PATH} \
    worker.reward.reward_type=batch \
    worker.reward.reward_function=./examples/reward_function/wmdp_strict.py:compute_score \
    ...
    trainer.experiment_name=llama38b_wmdp_grpo_alt_update \
    ...
    trainer.alt_update_summary_interval=20
